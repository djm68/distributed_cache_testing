...
int c, get_flag, del_flag, obj_max, bytes_max, loop_max, ttl;
...
int main(int argc, char *argv[])
{
  ...
  while ((c = getopt(argc, argv, ":hgdo:b:l:t:")) != -1) {
      switch(c) {
      case 'h':
          usage();
          break;
...
void usage()
{
  printf("put_get_del -h -g -d -o num -l num -b num -t num\n");
      ...
      while ( obj_count < obj_max )
        {
          ...
          sprintf(key, "key_%s_%d_%d", host, pid, obj_count);
            ...
            //  hack -- div bytes/loop_count to make small, small obj_len
            obj_len = ( bytes_max / loop_count );
