...
int sleep_flag, nomd5_flag, nouniq_flag, norand_flag, noget_flag, del_flag,
  obj_max, bytes_max, loop_max, ttl, num_threads, put_get_ratio;
extern char *optarg;
...
void parse_args(int argc, char *argv[])
{
  ...
  if (argc < 2)
  usage();
  while ((c = getopt(argc, argv, ":VURGhdso:b:g:l:t:r:p:")) != -1) {
    switch(c) 
    {
      case 'h':
        usage();
...
void *cache_operations(void *num)
{
  ...
  for (i=0; i < obj_max; ++i)            /* mem for m5 hashes, 32 chars +1 \0 */
  ...
  while ( loop_count < loop_max )                                /* main loop */
  {
    ...
    {
      if (nouniq_flag)   /* obj key w/o loop num -  will be reused in loop 2+ */
      ...
      printf("tid %ld ", tid);
      if ( obj_count == 0 )    /* object for obj_count 0 - save rand str time */
        len = gen_rand_str(big_obj);

      /* replace first element with (char) of obj_count - uniqs each obj */ 
      big_obj[0]= (char) obj_count;
      ...
      /* Save the md5 hash in md5_arr (32+1 for \0) */
      strncpy(md5_arr[obj_count], obj_md5, 33);
      ++obj_count;
...
int rna_get(char *key, void *buffer, uint64_t buffer_size, uint64_t object_size)
{
  ...
  else if (rc < 0)
  {
    fail_flag = rc;
    if (rc == -ENOENT)
      fprintf(stderr, "get: Object %s does not exist\n", key);
    else if (rc == -EFBIG)
      fprintf(stderr, "GET error, buffer too small for object %s of size %d rc %d\n", key, object_size, rc);
  else
      printf("Non specific GET error key: %s of size %d with rc %d\n", key, object_size, rc);
  }
...
static const char text[] = "abcdefghijklmnopqrstuvwxyz0123456789"
  ...
  if (norand_flag)                                        /* Rand size fixed? */ 
